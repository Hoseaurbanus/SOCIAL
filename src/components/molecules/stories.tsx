...
interface StoriesProps { stories: Story[]; onStoryClick: (id: string) => void; onAddStory: () => void }

export function Stories({ stories, onStoryClick, onAddStory }: StoriesProps) {
  return (
    <div className="flex gap-4 p-4 overflow-x-auto">
      <button onClick={onAddStory} className="flex flex-col items-center gap-1">
      ...
      {stories.map((story) => (
        <button key={story.id} onClick={() => onStoryClick(story.id)} className="flex flex-col items-center gap-1">
